...
const command: Command = {
  ...
  async execute(client, interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
      ...
      const buildView = async (view: ViewMode) => this.buildViewPayload(fixtureId, fixture, view);

      const { embed, row } = await buildView(currentView);
      ...
      collector.on("collect", async (buttonInteraction) => {
        try {
          ...
          const { embed: updatedEmbed, row: updatedRow } = await this.buildViewPayload(
            fixtureId,
            freshFixture,
            currentView,
          );
  ...
  async buildViewPayload(fixtureId: number, fixture: FootballFixture, view: ViewMode) {
    ...
    switch (view) {
      case "stats":
        return { embed: await this.buildStatsEmbed(fixtureId, fixture), row };
      case "lineups":
        return { embed: await this.buildLineupsEmbed(fixtureId, fixture), row };
      case "timeline":
        return { embed: await this.buildTimelineEmbed(fixtureId, fixture), row };
      default:
        return { embed: this.buildOverviewEmbed(fixture), row };
    }
  },

  buildOverviewEmbed(fixture: FootballFixture) {
    const timeDisplay =
  ...
  async buildStatsEmbed(fixtureId: number, fixture: FootballFixture) {
    try {
      ...
      const fields = stats.slice(0, 2).map((teamStats: unknown) => {
        const typed = teamStats as { team?: { name: string }; statistics?: { type: string; value: string | number | null }[] };
  ...
  async buildLineupsEmbed(fixtureId: number, fixture: FootballFixture) {
    try {
  ...
  async buildTimelineEmbed(fixtureId: number, fixture: FootballFixture) {
    try {
