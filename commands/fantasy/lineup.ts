...
function autoPickStartingXI(players: IFantasyPlayer[]): IFantasyPlayer[] {
  ...
  const def = sortByValue(byPosition.DEF).slice(0, 4);
  const mid = sortByValue(byPosition.MID).slice(0, 4);
  const fwd = sortByValue(byPosition.FWD).slice(0, 2);
...
const command: Command = {
  data: new SlashCommandBuilder()
    ...
    .addBooleanOption((option) =>
      option
        .setName("autopick")
        .setDescription("Auto-pick your strongest valid XI based on price")
        .setRequired(false),
  ...
  async execute(client, interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
      ...
      const workingPlayers: IFantasyPlayer[] = autopick ? autoPickStartingXI(team.players) : team.players;
      ...
      const grouped = groupByPosition(workingPlayers);
      const validation = validateFormation(workingPlayers);

      const embed = buildLineupEmbed(team.teamName, grouped, validation, autopick);
